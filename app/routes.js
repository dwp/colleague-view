...
// this is for NINO number for Demo prototype 
router.post('/for-demo-only/opt1/engagement-type', function (req, res) {
  ...
  if (req.session.data['national-insurance-number-for-demo'] == 'QQ123456Q' || req.session.data['national-insurance-number-for-demo'] == 'qq123456q') {
    // Send user to next page
    res.render('for-demo-only/opt1/engagement-type')
  } else {
    var errMsg = "";
    if (req.session.data['national-insurance-number-for-demo'] == '' || req.session.data['national-insurance-number-for-demo'] == undefined) {
      errMsg = "National Insurance Number is mandatory";
    ...
    // Send user to error page
    res.render('for-demo-only/opt1/error-NINO-number', { "errMsg": errMsg });
  }
})


// this is for radio selection
router.post('/for-demo-only/engagement-with', function (req, res) {
  ...
  if (howManyBalls == "Incoming telephone call" || howManyBalls == "Outgoing telephone call") {
    // Send user to next page
    res.redirect('/for-demo-only/engagement-with');
  } else {
    req.session.data['What-type-of-engagement-is-it-demo'] = '';
    // Send user back to same page
    res.redirect('back');
...
// this is for engagement with radio selection
router.post('/for-demo-only/home-page', function (req, res) {
  ...
  if (engagemetWith == "Not Listed above") {
    ...
    // Send user to next page
    res.redirect('/for-demo-only/home-page');
  }
...
// Select Context, subcontext and outcome page for Sprint23

// this is all about opt1 - (Design iteration1)

// this is for NINO number

  router.post('/sprint23/opt1/engagement-type', function (req, res) {
    req.session.data['What-type-of-engagement-is-it-sprint27'] = '';
    req.session.data['Who-is-the-engagement-with-sprint27'] = '';
    req.session.data['Who-is-the-engagement-with'] = '';
    req.session.data['Do-you-want-to-complete-the-session'] = '';
  ...
  if (req.session.data['national-insurance-number-sprint23'] == 'QQ123456Q' || req.session.data['national-insurance-number-sprint23'] == 'qq123456q') {
    // Send user to next page
    res.render('sprint23/opt1/engagement-type');
  } else {
    var errMsg = "";
    ...
    // Send user to error page
    res.render('sprint23/opt1/error-NINO-number', { "errMsg": errMsg });
  }
})

// for returning user
router.post('/sprint23/opt1/returning-user/engagement-type', function (req, res) {
  req.session.data['What-type-of-engagement-is-it-sprint27'] = '';
  req.session.data['Who-is-the-engagement-with-sprint27'] = '';
  req.session.data['Who-is-the-engagement-with'] = '';
...
if (req.session.data['national-insurance-number-sprint23'] == 'QQ123456Q' || req.session.data['national-insurance-number-sprint23'] == 'qq123456q') {
  // Send user to next page
  res.render('sprint23/opt1/returning-user/engagement-type');
} else {
  var errMsg = "";
  if (req.session.data['national-insurance-number-sprint23'] == '' || req.session.data['national-insurance-number-sprint23'] == undefined) {
    errMsg = "National Insurance Number is mandatory";
...
// this is for radio selection
router.post('/sprint23/opt1/engagement-with', function (req, res) {
  ...
  if (howManyBalls == "Incoming telephone call" || howManyBalls == "Outgoing telephone call") {
    // Send user to next page
    res.redirect('/sprint23/opt1/engagement-with');
  } else {
    req.session.data['What-type-of-engagement-is-it-sprint27'] = '';
    // Send user back to same page
    res.redirect('back');
...
router.post('/sprint23/opt1/home-page-first-time', function (req, res) {
  // Make a variable and give it the value from 'how-many-balls'
  if (req.session.data['Who-is-the-engagement-with'] == "Christopher Fox" || req.session.data['Who-is-the-engagement-with'] == "Jane Doe") {
    // Send user to next page
    req.session.data['Who-is-the-engagement-with'] = "with " + req.session.data['Who-is-the-engagement-with'];
    res.redirect('/sprint23/opt1/home-page-first-time');
  } else {
    // Send user back to same page
    // res.redirect('/sprint23/opt1/wrong-option-selected');
    req.session.data['Who-is-the-engagement-with'] = '';
    res.redirect('back');
  }
...
router.post('/sprint23/opt1/engagement-log-journey/session-completed', function (req, res) {
  var completeSession = req.session.data['Do-you-want-to-complete-the-session']

  // Check whether the variable matches a condition
  if (completeSession == "Yes") {
    // Send user to next page
    res.redirect('/sprint23/opt1/contact-history-callSummeryAddedd#add-to-contact-history');
  } else {
    res.redirect('/sprint23/opt1/engagement-log-journey/session-completed');
  }

  // New code suggested by Bhavin start here
  // if (req.session.data['What-services-have-they-called-about'].includes('esa')) {
  //   res.redirect('/sprint23/opt1/engagement-log-journey/session-completed');
...
router.post('/sprint23/opt1/returning-user/contact-history', function (req, res) {
  var completeSession = req.session.data['Do-you-want-to-complete-the-session']

  // Check whether the variable matches a condition
  if (completeSession == "Yes") {
    // Send user to next page
    res.redirect('/sprint23/opt1/returning-user/contact-history');
  } else {
    res.redirect('/sprint23/opt1/returning-user/confirmation-complete-session-Error');
  }

});

router.post('/sprint23/opt1/engagement-log-journey/benefitPages', function (req, res) {
  var pageLength = '';
  var isEsa = req.session.data['What-services-have-they-called-about'].includes('esa');
  var isPip = req.session.data['What-services-have-they-called-about'].includes('pip');
  var isCa = req.session.data['What-services-have-they-called-about'].includes('ca');
  ...
  if (isEsa && isPip) {
    res.render('sprint23/opt1/engagement-log-journey/what-queries-dealt-with-ESA', {
      "nextUrl": '/sprint23/opt1/engagement-log-journey/what-queries-dealt-with-PIP',
      "pageLength": pageLength
    });
  }
  if (isEsa && isCa && !isPip) {
    // console.log('considition executed isEsa && isCa && !isPip');
    res.render('sprint23/opt1/engagement-log-journey/what-queries-dealt-with-ESA', {
      "nextUrl": '/sprint23/opt1/engagement-log-journey/what-queries-dealt-with-CA',
      "pageLength": pageLength
    });
  }
  if (isEsa && !isPip && !isCa) {
    // console.log('considition executed isEsa && !isPip && !isCa');
    res.render('sprint23/opt1/engagement-log-journey/what-queries-dealt-with-ESA', {
      "nextUrl": '/sprint23/opt1/engagement-log-journey/outcome-queries',
      "pageLength": pageLength
    });
  }
  if (!isEsa && isPip && isCa) {
    // console.log('considition executed !isEsa && isPip && isCa');
    res.render('sprint23/opt1/engagement-log-journey/what-queries-dealt-with-PIP', {
      "nextUrl": '/sprint23/opt1/engagement-log-journey/what-queries-dealt-with-CA',
      "pageLength": pageLength
    });
  }
  ...
  if (!isEsa && !isPip && isCa) {
    // console.log('considition executed !isEsa && !isPip && isCa');
    res.render('sprint23/opt1/engagement-log-journey/what-queries-dealt-with-CA', {
      "nextUrl": '/sprint23/opt1/engagement-log-journey/outcome-queries',
      "pageLength": pageLength
    });
  }
...
router.post('/sprint23/opt1/engagement-log-journey/what-queries-dealt-with-PIP', function (req, res) {
  var isPip = req.session.data['What-services-have-they-called-about'].includes('pip');
  var isCa = req.session.data['What-services-have-they-called-about'].includes('ca');
  var pageLength = '';
  if (req.session.data['What-services-have-they-called-about'].length == 1) {
...
router.post('/sprint23/opt1/engagement-log-journey/what-queries-dealt-with-CA', function (req, res) {
  var pageLength = '';
  if (req.session.data['What-services-have-they-called-about'].length == 1) {
    pageLength = 'Services 1 of 1';
  } else if (req.session.data['What-services-have-they-called-about'].length == 2) {
...
router.post('/sprint23/opt1/engagement-log-journey/outcome-queries', function (req, res) {
  ...
  res.render('sprint23/opt1/engagement-log-journey/outcome-queries', {
...
router.get('/sprint23/opt1/engagement-log-journey/add-note/:name', function (req, res) {
  // console.log(' Add Notes route is called - GET: ',req.params.name);
  res.render('sprint23/opt1/engagement-log-journey/add-note', {
    "notesName": req.params.name,
    "notesValue": req.session.data[req.params.name]
  });
});
...
// this is all about opt2 - (Design iteration3)

// this is for NINO number

router.post('/sprint23/opt2/engagement-type', function (req, res) {
  req.session.data['What-type-of-engagement-is-it-sprint27'] = '';
  req.session.data['Who-is-the-engagement-with-sprint27'] = '';
  req.session.data['Who-is-the-engagement-with'] = '';
  req.session.data['Do-you-want-to-complete-the-session'] = '';
...
if (req.session.data['national-insurance-number-sprint23'] == 'QQ123456Q' || req.session.data['national-insurance-number-sprint23'] == 'qq123456q') {
  // Send user to next page
  res.render('sprint23/opt2/engagement-type');
} else {
  var errMsg = "";
  if (req.session.data['national-insurance-number-sprint23'] == '' || req.session.data['national-insurance-number-sprint23'] == undefined) {
    errMsg = "National Insurance Number is mandatory";
  ...
  // Send user to error page
  res.render('sprint23/opt2/error-NINO-number', { "errMsg": errMsg });
}
})

// for returning user
router.post('/sprint23/opt2/returning-user/engagement-type', function (req, res) {
req.session.data['What-type-of-engagement-is-it-sprint27'] = '';
req.session.data['Who-is-the-engagement-with-sprint27'] = '';
req.session.data['Who-is-the-engagement-with'] = '';
...
if (req.session.data['national-insurance-number-sprint23'] == 'QQ123456Q' || req.session.data['national-insurance-number-sprint23'] == 'qq123456q') {
// Send user to next page
res.render('sprint23/opt2/returning-user/engagement-type');
} else {
var errMsg = "";
if (req.session.data['national-insurance-number-sprint23'] == '' || req.session.data['national-insurance-number-sprint23'] == undefined) {
  errMsg = "National Insurance Number is mandatory";
...
// this is for radio selection
router.post('/sprint23/opt2/engagement-with', function (req, res) {
...
if (howManyBalls == "Incoming telephone call" || howManyBalls == "Outgoing telephone call") {
  // Send user to next page
  res.redirect('/sprint23/opt2/engagement-with');
} else {
  req.session.data['What-type-of-engagement-is-it-sprint27'] = '';
  // Send user back to same page
  res.redirect('back');
...
router.post('/sprint23/opt2/home-page-first-time', function (req, res) {
// Make a variable and give it the value from 'how-many-balls'
if (req.session.data['Who-is-the-engagement-with'] == "Christopher Fox" || req.session.data['Who-is-the-engagement-with'] == "Jane Doe") {
  // Send user to next page
  req.session.data['Who-is-the-engagement-with'] = "with " + req.session.data['Who-is-the-engagement-with'];
  res.redirect('/sprint23/opt2/home-page-first-time');
} else {
  // Send user back to same page
  // res.redirect('/sprint23/opt2/wrong-option-selected');
  req.session.data['Who-is-the-engagement-with'] = '';
  res.redirect('back');
}
...
router.post('/sprint23/opt2/engagement-log-journey/confirmation-complete-session.html', function (req, res) {
res.redirect('/sprint23/opt2/engagement-log-journey/confirmation-complete-session');
});


// this is for radio selection for Session complete
router.post('/sprint23/opt2/engagement-log-journey/session-completed', function (req, res) {
var completeSession = req.session.data['Do-you-want-to-complete-the-session']

// Check whether the variable matches a condition
if (completeSession == "Yes") {
  // Send user to next page
  res.redirect('/sprint23/opt2/contact-history-callSummeryAddedd#add-to-contact-history');
} else {
  res.redirect('/sprint23/opt2/engagement-log-journey/session-completed');
}

});

// this is for radio selection for Session complete when no notes added.
router.post('/sprint23/opt2/returning-user/contact-history', function (req, res) {
var completeSession = req.session.data['Do-you-want-to-complete-the-session']

// Check whether the variable matches a condition
if (completeSession == "Yes") {
  // Send user to next page
  res.redirect('/sprint23/opt2/returning-user/contact-history');
} else {
  res.redirect('/sprint23/opt2/returning-user/confirmation-complete-session-Error');
}

});

router.post('/sprint23/opt2/engagement-log-journey/benefitPages', function (req, res) {
...
var isCa = req.session.data['What-services-have-they-called-about'].includes('ca');
if (req.session.data['What-services-have-they-called-about'].length == 1) {
  pageLength = 'Benefit 1 of 1';
} else if (req.session.data['What-services-have-they-called-about'].length == 2) {
  pageLength = 'Benefits 1 of 2';
} else {
  pageLength = 'Benefits 1 of 3';
}

if (isEsa && isPip) {
  res.render('sprint23/opt2/engagement-log-journey/what-queries-dealt-with-ESA', {
    "nextUrl": '/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-PIP',
    "pageLength": pageLength
  });
}
if (isEsa && isCa && !isPip) {
  // console.log('considition executed isEsa && isCa && !isPip');
  res.render('sprint23/opt2/engagement-log-journey/what-queries-dealt-with-ESA', {
    "nextUrl": '/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-CA',
    "pageLength": pageLength
  });
}
if (isEsa && !isPip && !isCa) {
  // console.log('considition executed isEsa && !isPip && !isCa');
  res.render('sprint23/opt2/engagement-log-journey/what-queries-dealt-with-ESA', {
    "nextUrl": '/sprint23/opt2/engagement-log-journey/outcome-queries',
    "pageLength": pageLength
  });
}
if (!isEsa && isPip && isCa) {
  // console.log('considition executed !isEsa && isPip && isCa');
  res.render('sprint23/opt2/engagement-log-journey/what-queries-dealt-with-PIP', {
    "nextUrl": '/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-CA',
    "pageLength": pageLength
  });
}
...
if (!isEsa && !isPip && isCa) {
  // console.log('considition executed !isEsa && !isPip && isCa');
  res.render('sprint23/opt2/engagement-log-journey/what-queries-dealt-with-CA', {
    "nextUrl": '/sprint23/opt2/engagement-log-journey/outcome-queries',
    "pageLength": pageLength
  });
}
...
router.post('/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-PIP', function (req, res) {
var isPip = req.session.data['What-services-have-they-called-about'].includes('pip');
var isCa = req.session.data['What-services-have-they-called-about'].includes('ca');
var pageLength = '';
if (req.session.data['What-services-have-they-called-about'].length == 1) {
  ...
  pageLength = 'Benefits 2 of 2';
} else {
  pageLength = 'Benefits 2 of 3';
}
if (isPip && isCa) {
  // console.log('considition executed !isEsa && isPip && isCa');
  res.render('sprint23/opt2/engagement-log-journey/what-queries-dealt-with-PIP', {
...
router.post('/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-CA', function (req, res) {
var pageLength = '';
if (req.session.data['What-services-have-they-called-about'].length == 1) {
  pageLength = 'Benefit 1 of 1';
} else if (req.session.data['What-services-have-they-called-about'].length == 2) {
...
router.post('/sprint23/opt2/engagement-log-journey/outcome-queries', function (req, res) {
...
res.render('sprint23/opt2/engagement-log-journey/outcome-queries', {
...
router.post('/prototype-sprint-wise/sprint23/opt1/latestRedesignScreens/questionAnswered', function(req, res) {

  if (req.session.data['questionAsk_esa'] == '' || req.session.data['questionAsk_esa'] == undefined) {
        res.redirect("/prototype-sprint-wise/sprint23/opt1/latestRedesignScreens/questionAsk_esa-Error")
    } else {
      ...
      if(req.session.data['questionAsk_esa'].includes('Reason for a change in payment')) {
        ...
        res.redirect("/prototype-sprint-wise/sprint23/opt1/latestRedesignScreens/questionAnswered")
    }
})

router.post('/prototype-sprint-wise/sprint23/opt1/latestRedesignScreens/checkAnswer', function(req, res) {
  ...
  if (req.session.data['npd_wasQuestionResolved'] == 'Resolved' || req.session.data['npd_wasQuestionResolved'] == 'Not resolved' && req.session.data['npa_wasQuestionResolved'] == 'Resolved' || req.session.data['npa_wasQuestionResolved'] == 'Not resolved' ) {
        res.redirect("/prototype-sprint-wise/sprint23/opt1/latestRedesignScreens/checkAnswer")
    } else {
        res.redirect("/prototype-sprint-wise/sprint23/opt1/latestRedesignScreens/questionAnswered-Error")
    }
})
...
if (completeSession == "Yes") {
  // Send user to next page
  res.redirect('/prototype-sprint-wise/sprint23/opt2/returning-user/contact-history');
} else {
  res.redirect('/prototype-sprint-wise/sprint23/opt2/returning-user/confirmation-complete-session-Error');
}
...
if (isEsa && isPip) {
  res.render('prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-ESA', {
    "nextUrl": '/prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-PIP',
    "pageLength": pageLength
  });
}
if (isEsa && isCa && !isPip) {
  // console.log('considition executed isEsa && isCa && !isPip');
  res.render('prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-ESA', {
    "nextUrl": '/prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-CA',
    "pageLength": pageLength
  });
}
if (isEsa && !isPip && !isCa) {
  // console.log('considition executed isEsa && !isPip && !isCa');
  res.render('prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-ESA', {
    "nextUrl": '/prototype-sprint-wise/sprint23/opt2/engagement-log-journey/outcome-queries',
    "pageLength": pageLength
  });
}
if (!isEsa && isPip && isCa) {
...
router.post('/prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-PIP', function (req, res) {
var isPip = req.session.data['What-services-have-they-called-about'].includes('pip');
var isCa = req.session.data['What-services-have-they-called-about'].includes('ca');
var pageLength = '';
if (req.session.data['What-services-have-they-called-about'].length == 1) {
...
router.post('/prototype-sprint-wise/sprint23/opt2/engagement-log-journey/what-queries-dealt-with-CA', function (req, res) {
var pageLength = '';
if (req.session.data['What-services-have-they-called-about'].length == 1) {
  pageLength = 'Services 1 of 1';
} else if (req.session.data['What-services-have-they-called-about'].length == 2) {
...
res.render('prototype-sprint-wise/sprint23/opt2/engagement-log-journey/outcome-queries', {
...
router.get('/prototype-sprint-wise/sprint23/opt2/engagement-log-journey/add-note/:name', function (req, res) {
// console.log(' Add Notes route is called - GET: ',req.params.name);
res.render('prototype-sprint-wise/sprint23/opt2/engagement-log-journey/add-note', {
  "notesName": req.params.name,
  "notesValue": req.session.data[req.params.name]
});
});
...
router.post('/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/questionAsk_esa', function(req, res) {

  if (req.session.data['whichBenefitDiscussed'] == 'We did not discuss a benefit') {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/unHappy_journey/add-Note")
    } else {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/questionAsk_esa")
    }
})

router.post('/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/questionAnswered', function(req, res) {

  if (req.session.data['questionAsk_esa'] == '' || req.session.data['questionAsk_esa'] == undefined) {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/questionAsk_esa-Error")
    } else {
      var isNpd = 'govuk-!-display-none';
      var isNpa = 'govuk-!-display-none';
      var isMp = 'govuk-!-display-none';
      ...
      if(req.session.data['questionAsk_esa'].includes('Reason for a change in payment')) {
        ...
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/questionAnswered")
    }
})

router.post('/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/checkAnswer', function(req, res) {

  if (req.session.data['npd_wasQuestionResolved'] == 'Resolved' || req.session.data['npd_wasQuestionResolved'] == 'Not resolved' && req.session.data['npa_wasQuestionResolved'] == 'Resolved' || req.session.data['npa_wasQuestionResolved'] == 'Not resolved' ) {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/checkAnswer")
    } else {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/questionAnswered-Error")
    }
})

// Unhappy scenario condition

router.post('/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/selectBenefit', function(req, res) {

  if (req.session.data['unHappy_discussAnthingElse'] == 'Yes') {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/selectBenefit")
    } else {
        res.redirect("/prototype-sprint-wise/sprint23/opt2/latestRedesignScreens/unHappy_journey/unHappy_summary_CallLogged")
