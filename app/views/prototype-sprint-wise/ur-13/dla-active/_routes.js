...
router.post('/index.html', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/index');
});
...
router.post('/why-checking-information', function (req, res) {
  ...
  if (
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/why-checking-info');
  } else {
    // var errMsg = '';
    if (
      ...
      res.redirect('/prototype-sprint-wise/ur-13/dla-active/error-nino-number');
    } else {
      res.redirect('back');
    }
  }
...
router.post('/call-with', function (req, res) {
  ...
  if (req.session.data['contact-types'] == 'A phone call') {
    if (req.session.data['What-type-of-contact'] == 'Inbound phone call with') {
      res.redirect('/prototype-sprint-wise/ur-13/dla-active/call-with');
    } else if (req.session.data['What-type-of-contact'] == 'Outbound phone call with') {
      res.redirect('/prototype-sprint-wise/ur-13/dla-active/was-call-answered');
    }
  }
  ...
  if (req.session.data['contact-types'] == 'A letter') {
    if (req.session.data['What-type-of-contact'] == 'Inbound letter from' || req.session.data['What-type-of-contact'] == 'Outbound letter to') {
      res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/contact-with');
    }
  }
 ...
 if (req.session.data['contact-types'] == 'View only') {
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/view-only/home');
  }

  if (req.session.data['What-type-of-contact'] == '') {
    console.log('Error page');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/error-why-checking-info');
  }
});
...
//     res.redirect('/prototype-sprint-wise/ur-13/dla-active/who-was-the-call-with');
//   }
//   else{
//     res.redirect('/prototype-sprint-wise/ur-13/dla-active/who-were-tryingTo-contact');
//   }
// });
...
router.post('/who-were-tryingTo-contact', function (req, res) {
  ...
  // Phone call conditions
  if (req.session.data['was-call-answered'] == 'Yes') {
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/home');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/what-benefits-discussed');
    // res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-log-outbound-call-attempt-failed');
  }
});
...
router.post('/non-telephony/home', function (req, res) {
  ...
  if (req.session.data['Who-is-contact-with'] == '') {
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/home');
  }
});
...
router.post('/add-call/what-benefits-discussed', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '' && req.session.data['Do-you-want-to-complete-the-call'] != '') {
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/do-you-want-add-more-details');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    req.session.data['addNote']= '';
...
router.post('/add-call/select-contact-type', function (req, res) {
 ...
 {
  console.log('Error page');
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/what-benefits-discussed-error');
  } else if (req.session.data['what-benefit-discussed'] == 'General information') {
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/general-information');
  } else {
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/select-contact-type');
 }
})
...
router.post('/add-call/questions-asked', function (req, res) {
  ...
  if (req.session.data['what-benefit-discussed'] == '')
 {
  ...
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/questions-asked');
 }
})

router.post('/add-call/questions-answered', function (req, res) {
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/questions-answered');
});
...
router.post('/add-call/is-question-resolved', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/is-question-resolved');
});

router.post('/add-call/added-call-details', function (req, res) {
  
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/added-call-details');
});
...
router.post('/add-call/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/added-details');
  }
});
...
router.post('/add-call/add-more-call-details', function (req, res) {
  ...
  if (addCallDetails == 'Complete phone call'){
    // res.redirect('/prototype-sprint-wise/ur-13/dla-active//add-call/call-completed');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/index')
  }
  if (addCallDetails == 'Change your note'){
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/change-notes');
  }
  if (addCallDetails == 'Add a note'){
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/add-a-note');
  }
});
...
router.post('/add-call/do-you-want-to-complete-call', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '') {
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/do-you-want-to-complete-call');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/add-call/no-contact-added');
  }
});
...
router.post('/non-telephony/add-contact/what-benefits-discussed', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] != '' && req.session.data['Do-you-want-to-complete-the-contact'] != '')  {
    // console.log('Display summary page');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/do-you-want-add-more-details');
  }
  else{
    // req.session.data['what-benefit-discussed-non-telephony'] = '';
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/what-benefits-discussed');
  }
});
...
router.post('/non-telephony/add-contact/select-contact-type', function (req, res) {
 ...
 {
  console.log('Error page');
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/what-benefits-discussed-error');
  } else {
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/select-contact-type');
 }
})
...
router.post('/non-telephony/add-contact/questions-asked', function (req, res) {
  ...
  if (req.session.data['what-benefit-discussed-non-telephony'] == '')
 {
  ...
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/general-information');
} else {
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/questions-asked');
 }
})
...
router.post('/non-telephony/add-contact/questions-answered', function (req, res) {
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/questions-answered');
});
...
router.post('/non-telephony/add-contact/is-question-resolved', function (req, res) {
  ...
  req.session.data['chpa_question-resolved']= '';
  req.session.data['othQ_question-resolved']= '';
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/is-question-resolved');
});

router.post('/non-telephony/add-contact/added-contact-details', function (req, res) {
  
  res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/added-contact-details');
});
...
router.post('/non-telephony/add-contact/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    console.log('Add Notes');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/added-details');
  }
});

router.post('/non-telephony/add-contact/check-before-start-sercive-again', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] == ''){
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/check-before-start-sercive-again');
  } else{
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/do-you-want-to-complete-contact'); 
  }
});
...
router.post('/non-telephony/add-contact/contact-details-added', function (req, res) {
  ...
  if (addContactDetails == 'Yes'){
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/contact-completed')
  }
});
...
router.post('/non-telephony/add-contact/add-more-contact-details', function (req, res) {
  ...
  if (addCallDetails == 'Change your note'){
    ...
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/change-notes');
  }
  if (addCallDetails == 'Add a note'){
    console.log('Add a note');
    res.redirect('/prototype-sprint-wise/ur-13/dla-active/non-telephony/add-contact/add-a-note');
  }
});
