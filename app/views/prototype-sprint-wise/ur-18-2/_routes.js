...
router.post('/index.html', function (req, res) {
  res.redirect('/prototype-sprint-wise/ur-18-2/index');
});
...
router.post('/working-on', function (req, res) {
  ...
  if (
    ...
    res.render('/prototype-sprint-wise/ur-18-2/working-on');
  } else {
    // var errMsg = '';
    if (
      ...
      res.redirect('/prototype-sprint-wise/ur-18-2/error-nino-number');
    } else {
      res.redirect('back');
    }
  }
...
router.post('/call-with', function (req, res) {
  ...
  if (req.session.data['contact-types'] == 'A phone call') {
      res.redirect('/prototype-sprint-wise/ur-18-2/call-with');
  }

  if (req.session.data['contact-types'] == 'A letter') {
    if (req.session.data['What-type-of-contact'] == 'Inbound letter from') {
      res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/letter-from');
    } else if (req.session.data['What-type-of-contact'] == 'Outbound letter to') {
      res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/letter-to');
    }
  }

  if (req.session.data['contact-types'] == 'An email') {
    if (req.session.data['What-type-of-contact'] == 'Inbound email from') {
      res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/email-from');
    } else if (req.session.data['What-type-of-contact'] == 'Outbound email to') {
      res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/email-to');
    }
  }

  // Notification condition
  if (req.session.data['contact-types'] == 'Text message to') {
      res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/message-to');
    }
  ...
  if (req.session.data['contact-types'] == 'A visit or appointment') {
      if (req.session.data['What-type-of-contact'] == 'Visit from' || req.session.data['What-type-of-contact'] == 'Visit to') {
        res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/visit-with');
      }
  }

  // view only
 if (req.session.data['contact-types'] == 'View only') {
    res.redirect('/prototype-sprint-wise/ur-18-2/view-only/home');
  }
...
router.post('/telephony/add-call/planned-for-review', function (req, res) {
  ...
  if (req.session.data['was-call-answered'] == 'Yes') {
    // req.session.data['Who-is-the-phone-call-with-ur8'] = '';
     res.redirect('/prototype-sprint-wise/ur-18-2/telephony/home');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    req.session.data['addNote']= '';
    ...
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/reviewed');
  }
});
...
router.post('/telephony/home', function (req, res) {
  ...
  if (
    ...
    if (req.session.data['What-type-of-contact'] == 'Inbound phone call with') {
        ...
        res.redirect('/prototype-sprint-wise/ur-18-2/call-status');
      }
});
...
router.post('/non-telephony/home', function (req, res) {
  ...
  if (req.session.data['Who-is-contact-with'] == '') {
    ...
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/home');
  }
});
...
router.post('/telephony/add-call/reviewed', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '' && req.session.data['Do-you-want-to-complete-the-call'] != '') {
    ...
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/do-you-want-add-more-details');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    req.session.data['addNote']= '';
...
router.post('/telephony/add-call/planned-action', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/reviewed-error');
  } else {
...
router.post('/telephony/add-call/questions-answered', function (req, res) {
  res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/questions-answered');
});
...
router.post('/telephony/add-call/is-question-resolved', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/is-question-resolved');
});
...
router.post('/telephony/add-call/added-call-details', function (req, res) {
    ...
    //   res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/select-contact-type-error');
    // }else{
    //   res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/added-call-details');
    // }
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/added-call-details');
...
router.post('/telephony/add-call/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    ...
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-note')
  } else{
    // console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/added-details');
  }
});
...
router.post('/telephony/add-call/add-more-call-details', function (req, res) {
  ...
  if (addCallDetails == 'Complete phone call'){
    // res.redirect('/prototype-sprint-wise/ur-18-2//add-call/call-completed');
    res.redirect('/prototype-sprint-wise/ur-18-2/index')
  }
  if (addCallDetails == 'Change your note'){
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/change-notes');
  }
  if (addCallDetails == 'Add a note'){
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-a-note');
  }
});
...
router.post('/telephony/add-call/complete-call', function (req, res) {
  ...
  if (checkCallCompletion == 'Complete phone call'){
    ...
    // res.redirect('/prototype-sprint-wise/ur-18-2//telephony/add-call/call-completed')
    res.redirect('/prototype-sprint-wise/ur-18-2/index')
...
router.post('/telephony/add-call/do-you-want-to-complete-call', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '') {
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/do-you-want-to-complete-call');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/no-contact-added');
  }
});
...
router.post('/non-telephony/add-contact/reviewed', function (req, res) {
  ...
  if (req.session.data['contact-types'] == 'Text message to'){
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/message-about');
  } else if(checkVisistType && visitTo){
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/visit-about');
  } else {
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/reviewed');
  }
...
router.post('/non-telephony/add-contact/do-you-want-add-more-details', function (req, res) {
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/do-you-want-add-more-details');
});
...
router.post('/non-telephony/add-contact/select-contact-type', function (req, res) {
  ...
  if (req.session.data['what-benefit-discussed-non-telephony'] == '')
 {
  ...
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/reviewed-error');
  } else {
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/select-contact-type');
 }
})
...
router.post('/non-telephony/add-contact/questions-asked', function (req, res) {
  ...
  if (req.session.data['what-benefit-discussed-non-telephony'] == '')
 {
  ...
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/general-information');
} else {
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/questions-asked');
 }
})
...
router.post('/non-telephony/add-contact/questions-answered', function (req, res) {
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/questions-answered');
});
...
router.post('/non-telephony/add-contact/is-question-resolved', function (req, res) {
  ...
  req.session.data['chpa_question-resolved']= '';
  req.session.data['othQ_question-resolved']= '';
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/is-question-resolved');
});

router.post('/non-telephony/add-contact/added-contact-details', function (req, res) {
  
  res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/added-contact-details');
});
...
router.post('/non-telephony/add-contact/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    console.log('Add Notes');
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/added-details');
  }
});

router.post('/non-telephony/add-contact/check-before-start-sercive-again', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] == ''){
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/check-before-start-sercive-again');
  } else{
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/do-you-want-to-complete-contact'); 
  }
});
...
router.post('/non-telephony/add-contact/add-more-contact-details', function (req, res) {
  ...
  if (addCallDetails == 'Change your note'){
    ...
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/change-notes');
  }
  if (addCallDetails == 'Add a note'){
    console.log('Add a note');
    res.redirect('/prototype-sprint-wise/ur-18-2/non-telephony/add-contact/add-a-note');
  }
});
...
router.post('/non-telephony/add-contact/contact-details-completed', function (req, res) {
  ...
  if (addContactDetails == 'Yes'){
    // res.redirect('/prototype-sprint-wise/ur-18-2//non-telephony/add-contact/contact-completed')
    res.redirect('/prototype-sprint-wise/ur-18-2/index')
...
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/reviewed-dla-active', function (req, res) {
    req.session.data['what-benefit-discussed'] = '';
    req.session.data['addNote']= '';
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/reviewed-dla-active');
});
...
router.post('/telephony/add-call/option-b/planned-action', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/reviewed-error');
    ...
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/general-information');
  } else {
  res.render('/prototype-sprint-wise/ur-18-2/telephony/add-call/option-b/select-contact-type');
 }
})
router.post('/telephony/add-call/option-b/added-call-details', function (req, res) {
  
  res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/option-b/added-call-details');
});
router.post('/telephony/add-call/option-b/added-details', function (req, res) {
  var addNote = req.session.data['do-you-want-add-note'];
  if(addNote =='Yes'){
    // console.log('Add Notes');
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/option-b/add-note')
  } else{
    // console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/option-b/added-details');
  }
});
...
// recommended
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/added-details', function (req, res) {
  const addNote = req.session.data['do-you-want-add-note'];
  if (addNote === 'Yes') {
    return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-note');
  }
  // move forward instead of looping back to itself
  return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/complete-call');
});


router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/you-have-added-details', function (req, res) {
    console.log('Added details');
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/added-details')
});

router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-more-call-details', function (req, res) {
  var addCallDetails = req.session.data['do-you-want-to-add-more-call-detail'];
  if (addCallDetails == 'Yes I want to add'){
    req.session.data['what-benefit-discussed'] = '';
    res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/reviewed')
...
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/complete-call', function (req, res) {
  const choice = req.session.data['Do-you-want-to-complete-the-call'];

  if (choice === 'Complete phone call') {
    // Start service again
    return res.redirect('/prototype-sprint-wise/ur-18-2/index');
  }

  if (choice === 'back to home page') {
    // Continue working on this call -> Home
    return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/home');
  }
...
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/do-you-want-to-complete-call', function (req, res) {
  if (req.session.data['what-benefit-discussed']) {
    // proceed to decision handler, not back to itself
    return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/complete-call');
  }
  req.session.data['what-benefit-discussed'] = '';
  return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/no-contact-added');
});
...
// Save and continue
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/select-contact-type', (req, res) => {
  // Continue your normal flow (e.g., go to contact type selection page)
  return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/select-contact-type'); // or the correct next step
});

// Add or update other needs
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-additional-support-needs', (req, res) => {
  // Go to the page where user can add/update needs
  return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-additional-support-needs');
});

// UR‑18‑2 helper: redirect POST → GET so auto-store-data persists
router.post('/prototype-sprint-wise/ur-18-2/telephony/add-call/check-additional-needs-3', function (req, res) {
  return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/check-additional-needs-3');
});

// additional support router for UR-18-2
router.post('/additional-support-router-2', function (req, res) {
  ...
  if (answer === 'remove') {
    return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/remove-additional-support-needs');
  }

  if (answer === 'both') {
    return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/add-additional-support-needs-2');
  }

  return res.redirect('/prototype-sprint-wise/ur-18-2/telephony/add-call/identified-additional-support-needs');
});
