...
router.post('/index.html', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/index');
});
...
router.post('/why-checking-information', function (req, res) {
  ...
  if (
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/why-checking-info');
  } else {
    // var errMsg = '';
    if (
      ...
      res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/error-nino-number');
    } else {
      res.redirect('back');
    }
  }
...
router.post('/call-with', function (req, res) {
  ...
  if (req.session.data['What-type-of-contact'] == 'Inbound phone call with') {
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/call-with');
  }

  if (req.session.data['What-type-of-contact'] == 'Outbound phone call with') {
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/was-call-answered');
  }

  // Email conditions
  if (req.session.data['What-type-of-contact'] == 'Inbound letter from' || req.session.data['What-type-of-contact'] == 'Outbound letter to') {
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/contact-with');
  }
...
// face to face converstaions condition
 ...
 if (req.session.data['What-type-of-contact'] == 'View only') {
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/view-only/home');
  }
 ...
 if (req.session.data['contact-type'] == 'View only') {
  ...
  if (req.session.data['What-type-of-contact'] == '') {
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/error-why-checking-info');
  }
});
...
router.post('/home', function (req, res) {
  ...
  if (req.session.data['Who-is-the-phone-call-with-ur8'] == '') {
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/home');
  }
});
...
router.post('/non-telephony/home', function (req, res) {
  ...
  if (req.session.data['Who-is-contact-with'] == '') {
    // Send user to error page
    res.redirect(
      '/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/contact-with'
    );
  } else {
    if (
    ...
    res.redirect(
      '/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/home'
    );
  }
});
...
router.post('/add-call/questions-asked', function (req, res) {
 ...
 {
  console.log('Error page');
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/what-benefits-discussed-error');
  } else if (req.session.data['what-benefit-discussed'] == 'General information') {
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/general-information');
  } else {
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/questions-asked');
 }
})

router.post('/add-call/questions-answered', function (req, res) {
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/questions-answered');
});
...
router.post('/add-call/is-question-resolved', function (req, res) {
  ...
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/is-question-resolved');
});

router.post('/add-call/added-call-details', function (req, res) {
  
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/added-call-details');
});
...
router.post('/add-call/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/added-details');
  }
});
...
router.post('/add-call/add-more-call-details', function (req, res) {
  ...
  if (addCallDetails == 'Complete phone call'){
    // res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log//add-call/call-completed');
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/index')
  }
  if (addCallDetails == 'Change your note'){
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/change-notes');
  }
  if (addCallDetails == 'Add a note'){
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/add-a-note');
  }
});
...
router.post('/add-call/do-you-want-to-complete-call', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '') {
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/do-you-want-to-complete-call');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/add-call/no-contact-added');
  }
});
...
router.post('/non-telephony/add-contact/what-benefits-discussed', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] != '') {
    // console.log('Display summary page');
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/do-you-want-add-more-details');
  }
  else{
    // req.session.data['what-benefit-discussed-non-telephony'] = '';
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/what-benefits-discussed');
  }
});
...
router.post('/non-telephony/add-contact/questions-asked', function (req, res) {
 ...
 {
  console.log('Error page');
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/what-benefits-discussed-error');
 } else if (req.session.data['what-benefit-discussed-non-telephony'] == 'General information') {
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/general-information');
} else {
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/questions-asked');
 }
})
...
router.post('/non-telephony/add-contact/questions-answered', function (req, res) {
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/questions-answered');
});
...
router.post('/non-telephony/add-contact/is-question-resolved', function (req, res) {
  ...
  req.session.data['chpa_question-resolved']= '';
  req.session.data['othQ_question-resolved']= '';
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/is-question-resolved');
});

router.post('/non-telephony/add-contact/added-contact-details', function (req, res) {
  
  res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/added-contact-details');
});
...
router.post('/non-telephony/add-contact/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    console.log('Add Notes');
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/added-details');
  }
});

router.post('/non-telephony/add-contact/check-before-start-sercive-again', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] == ''){
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/check-before-start-sercive-again');
  } else{
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/do-you-want-to-complete-contact'); 
  }
});
...
router.post('/non-telephony/add-contact/contact-details-added', function (req, res) {
  ...
  if (addContactDetails == 'Yes'){
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/contact-completed')
  }
});
...
router.post('/non-telephony/add-contact/complete-contact', function (req, res) {
  ...
  if (checkCallCompletion == 'Complete phone contact'){
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/home')
  }
...
router.post('/non-telephony/add-contact/add-more-contact-details', function (req, res) {
  ...
  if (addCallDetails == 'Change your note'){
    ...
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/change-notes')
  }
  if (addCallDetails == 'Add a note'){
    console.log('Add a note');
    res.redirect('/prototype-sprint-wise/future-ur/failed-disconnect-call-log/non-telephony/add-contact/add-a-note')
  }
});
