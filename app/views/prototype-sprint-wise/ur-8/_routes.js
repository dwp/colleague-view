...
router.post('/a/why-checking-info', function (req, res) {
  ...
  // req.session.data['benefitList'] = ''; rahul
  // req.session.data['benefitList'] = '';
...
if (req.session.data['benefitList'].includes('esa')) {
res.redirect('/prototype-sprint-wise/ur-8/a/call-log-journey/confirmation-complete-session');
}else if (req.session.data['benefitList'].includes('pip')) {
res.redirect('/prototype-sprint-wise/ur-8/a/call-log-journey/confirmation-complete-session');
}if (req.session.data['benefitList'].includes('ca')) {
res.redirect('/prototype-sprint-wise/ur-8/a/call-log-journey/confirmation-complete-session');
}else {
...
if(req.session.data.outcomePage === '' || req.session.data.outcomePage === undefined){
...
req.session.data.outcomePage =outcomePageData;
  ...
  if (req.session.data['benefitList'].length >1) {
    console.log('Multiple benefits: ------------>',req.session.data['benefitList']);

    var isCA = req.session.data['benefitList'].includes("Carer Allowance");
    var isESA = req.session.data['benefitList'].includes("Employment and Support Allowance");
    var isPIP = req.session.data['benefitList'].includes("Personal Independence Payment");
    ...
    }  else if ( !isCA || !isESA || isPIP) {
      console.log('No one  selected from CA, ESA and PIP')
      res.redirect("/prototype-sprint-wise/ur-8/a/call-log-journey/benefits-discussed");
    }

  }
//   if( !isCA || !isPIP || !isESA) { 
//     console.log('This prototype is for testing only CA, ESA and PIP')
//     res.redirect("/prototype-sprint-wise/ur-8/a/call-log-journey/benefits-discussed");
// }
...
// Telephony user Question answer for multipme benefit
...
if (req.session.data['benefitList'].includes("Personal Independence Payment")){
...
router.post('/a/call-log-journey/added-call-details', function(req, res) {
  ...
  var benefitLenght = req.session.data['benefitList'].length;
...
outcomePageData.push({"benefit":req.session.data['benefitList'],
"question":item,
  ...
  res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/unHappy_journey/noBenefit/what-was-contact-about-notes")
    ...
    if (req.session.data['benefitList'].length >1) {
      ...
      if( isCA && isESA) { 
       ...
       req.session.data['chpa_wasQuestionResolved'] ='';
       req.session.data['othQ_wasQuestionResolved'] ='';
        res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-question-asked/ca-eas/ca-questions-asked");
      }
      ...
      if( isESA && isPIP) { 
        ...
        req.session.data['chpa_wasQuestionResolved'] ='';
        req.session.data['othQ_wasQuestionResolved'] ='';
        res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-question-asked/eas-pip/esa-questions-asked");
      } else if ( !isCA || !isESA || isPIP) {
        console.log('No one  selected from CA, ESA and PIP')
        res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-adding-details-for");
      }
...
router.post('/a/different-type-contact-user/call-log-journey/questions-outcomes', function(req, res) {
  ...
  if (req.session.data['questionAsk'].includes('Something else')) {
  res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/questions-outcomes-for-something-else")
  } else {
  if(req.session.data['questionAsk'].includes('Next payment date')) {
  isNpd = 'govuk-!-display-block';
  }
...
// ----for CA with ESA----
router.post('/a/different-type-contact-user/call-log-journey/benefit-question-asked/ca-eas/questions-outcomes-ca', function(req, res) {
  var isNpdCA = 'govuk-!-display-none';
  var isNpaCA = 'govuk-!-display-none';
  var isMpCA = 'govuk-!-display-none';
  var isRfchCA = 'govuk-!-display-none';
...
if (req.session.data['benefitList'].includes("Carer Allowance")){
  if (req.session.data['questionAsk-ca'].includes('Something else')) {
    ...
    //All variable render here
    res.render('prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-question-asked/ca-eas/questionAnswered-ca', {
      "isNpdCA": isNpdCA,
      "isNpaCA": isNpaCA,
      "isMpCA": isMpCA,
      "isRfchCA": isRfchCA,
...
// ----for ESA with CA----
router.post('/a/different-type-contact-user/call-log-journey/benefit-question-asked/ca-eas/questions-outcomes-esa', function(req, res) {
  var isNpdESA = 'govuk-!-display-none';
  var isNpaESA = 'govuk-!-display-none';
  var isMpESA = 'govuk-!-display-none';
  var isRfchESA = 'govuk-!-display-none';
...
if (req.session.data['benefitList'].includes("Employment and Support Allowance")){
  if (req.session.data['questionAsk-esa'].includes('Something else')) {
    ...
    //All variable render here
    res.render('prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-question-asked/ca-eas/questionAnswered-esa', {
      "isNpdESA": isNpdESA,
      "isNpaESA": isNpaESA,
      "isMpESA": isMpESA,
      "isRfchESA": isRfchESA,
...
// ----for ESA with CA----
router.post('/a/different-type-contact-user/call-log-journey/benefit-question-asked/eas-pip/questions-outcomes-esa', function(req, res) {
  var isNpdESA = 'govuk-!-display-none';
  var isNpaESA = 'govuk-!-display-none';
  var isMpESA = 'govuk-!-display-none';
  var isRfchESA = 'govuk-!-display-none';
...
if (req.session.data['benefitList'].includes("Employment and Support Allowance")){
  if (req.session.data['questionAsk-esa'].includes('Something else')) {
    ...
    //All variable render here
    res.render('prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-question-asked/eas-pip/questionAnswered-esa', {
      "isNpdESA": isNpdESA,
      "isNpaESA": isNpaESA,
      "isMpESA": isMpESA,
      "isRfchESA": isRfchESA,
...
// ----for PIP with ESA----
router.post('/a/different-type-contact-user/call-log-journey/benefit-question-asked/eas-pip/questions-outcomes-pip', function(req, res) {
  var isNpdPIP = 'govuk-!-display-none';
  var isNpaPIP = 'govuk-!-display-none';
  var isMpPIP = 'govuk-!-display-none';
  var isRfchPIP = 'govuk-!-display-none';
...
if (req.session.data['benefitList'].includes("Personal Independence Payment")){
  if (req.session.data['questionAsk-pip'].includes('Something else')) {
    ...
    //All variable render here
    res.render('prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/benefit-question-asked/eas-pip/questionAnswered-pip', {
      "isNpdPIP": isNpdPIP,
      "isNpaPIP": isNpaPIP,
      "isMpPIP": isMpPIP,
      "isRfchPIP": isRfchPIP,
...
// if ((req.session.data['npd_wasQuestionResolved'].includes('Not resolved') || req.session.data['npd_wasQuestionResolved'].includes('Resolved')) || (req.session.data['npa_wasQuestionResolved'].includes('Not resolved') || req.session.data['npa_wasQuestionResolved'].includes('Resolved')) || (req.session.data['ma_wasQuestionResolved'].includes('Not resolved') || req.session.data['ma_wasQuestionResolved'].includes('Resolved')) || (req.session.data['chpa_wasQuestionResolved'].includes('Not resolved') || req.session.data['chpa_wasQuestionResolved'].includes('Resolved')) || (req.session.data['othQ_wasQuestionResolved'].includes('Not resolved') || req.session.data['othQ_wasQuestionResolved'].includes('Resolved'))) {
//   console.log('Different benefit: check answer page');
...
router.post('/a/different-type-contact-user/call-log-journey/check-for-add-note', function(req, res) {
  if (req.session.data['benefitList'].length>1) {
    if (req.session.data['discussAnthingElse'] == 'Yes' ) {
      ...
      res.redirect('/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/added-details');
      }
  } else {
    //All variable render here
  let outcomePageData = [];
...
router.post('/a/different-type-contact-user/call-log-journey/do-you-want-to-complete-call', function(req, res) {
  res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/check-call-completion")
  })
...
// Do you want to complete the call?
router.post('/a/different-type-contact-user/call-log-journey/call-completed', function (req, res) {
  // Check whether the variable matches a condition
  var checkPhoneCallCompleted = req.session.data['Do you want to complete the call'];
  if (checkPhoneCallCompleted == 'back to home page') {
  res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/home");
  } else {
    res.redirect("/prototype-sprint-wise/ur-8/a/different-type-contact-user/call-log-journey/call-completed");
  }
  });
  ...
  router.post('/prototype-sprint-wise/ur-8/a/call-log-journey/benefit-question-asked/benefitPages', function (req, res) {
    var pageLength = '';
    var isAa = req.session.data['benefitList'].includes('Attendance Allowance');
    var isEsa = req.session.data['benefitList'].includes('Employment and Support Allowance');
    var isPip = req.session.data['benefitList'].includes('Personal Independence Payment');
    if (req.session.data['benefitList'].length == 1) {
      pageLength = 'Services 1 of 1';
    } else if (req.session.data['benefitList'].length == 2) {
...
//     console.log('lenght:', req.session.data['benefitList'].length);
//     let benefit = req.session.data['benefitList'];
//     console.log('Here is a new route defined by bhavin', benefit);

//     if (
//       req.session.data['benefitList'].includes("Carer's Allowance") &&
//       req.session.data['benefitList'].indexOf("Carer's Allowance") !=
//         0
//     ) {
//       const index =
//         req.session.data['benefitList'].indexOf("Carer's Allowance");
//       req.session.data['benefitList'].splice(index, 1);
//       console.log(
...
//       );
//     } else if (
//       req.session.data['benefitList'].includes('Employment and Support Allowance') &&
//       req.session.data['benefitList'].indexOf('Employment and Support Allowance') != 0
//     ) {
//       const index = req.session.data['benefitList'].indexOf('Employment and Support Allowance');
//       req.session.data['benefitList'].splice(index, 1);
//       console.log(
//         'Here is a new route defined by bhavin',
//         req.session.data['benefitList']
...
//       );
//     } else if (
//       req.session.data['benefitList'].includes(
//         'Personal Independence Payment'
//       ) &&
//       req.session.data['benefitList'].indexOf(
//         'Personal Independence Payment'
//       ) != 0
//     ) {
//       const index = req.session.data['benefitList'].indexOf(
//         'Personal Independence Payment'
//       );
//       req.session.data['benefitList'].splice(index, 1);
//       console.log(
...
//       console.log('check answer page number of benefits', req.session.data['benefitList'].length);
